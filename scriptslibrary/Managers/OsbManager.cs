...
public class OsbManager : Manager
{
    private Dictionary<string, string> variables = new Dictionary<string, string>();

    public void ImportOsb(string path)
    {
    ...
    private void parseVariablesSection(StreamReader reader)
        ...
        private void parseEventsSection(StreamReader reader)
        {
            ...
            reader.ParseSectionLines(line =>
            {
                ...
                switch (values[0])
                {
                    case "Sprite":
                        {
                            var layerName = GetLayerName(values[1]);
                    ...
                    case "Sample":
                        {
                            ...
                            var layerName = GetLayerName(values[2]);
                            var path = removePathQuotes(values[3]);
        ...
        private Layers GetLayerName(string layerString) => (Layers) Enum.Parse(typeof(Layers), layerString, false); 
